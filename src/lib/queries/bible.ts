...
export function getBibleVersions() {
  return prisma.bibleVersion.findMany({ orderBy: { id: "asc" } });
}

export function getBibleBooks(versionCode: string = DEFAULT_BIBLE_VERSION) {
  return prisma.bibleBook.findMany({
    where: { version: { code: versionCode } },
    orderBy: { order: "asc" },
  });
}

export function getBibleBookBySlug(slug: string, versionCode: string = DEFAULT_BIBLE_VERSION) {
  return prisma.bibleBook.findFirst({
    where: { slug, version: { code: versionCode } },
    include: {
...
export async function getBibleChapter(
  ...
  if (chapterNumber > 1) {
    ...
    const prevBook = await prisma.bibleBook.findFirst({
      where: { version: { code: versionCode }, order: book.order - 1 },
    });
    if (prevBook) {
...
export async function getRandomBibleVerse(
  ...
  const rows = await prisma.$queryRaw<RandomBibleVerse[]>`
    ...
    WHERE bb.versionId = (SELECT id FROM bible_versions WHERE code = ${versionCode})
    ORDER BY RANDOM()
    LIMIT 1
  `;
  return rows[0] ?? null;
...
export async function searchBible(
  ...
  return prisma.$queryRaw<BibleSearchResult[]>`
    ...
    WHERE bible_verses_fts MATCH ${ftsQuery} AND bb.versionId = (
      SELECT id FROM bible_versions WHERE code = ${versionCode}
    )
    ORDER BY rank
    LIMIT ${limit}
  `;
