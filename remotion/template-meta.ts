...
export type TemplateMeta<P> = {
  ...
  formats: FormatId[];
  /** Valeurs de départ ; accroche et appel à l'action dans la langue du Reel (textes d'exemple en français). */
  defaultProps: (format: FormatId, language?: ReelLanguage) => P;
...
const common = (format: FormatId, theme: ThemeId, language: ReelLanguage) => ({
  format,
...
const verset = define<VersetProps>({
  ...
  schema: versetSchema,
  spec: versetSpec,
  defaultProps: (format, language = "fr") => ({
    ...common(format, "Verset", language),
    kicker: label(language, "kickerVerset"),
...
const priere = define<PriereProps>({
  ...
  spec: priereSpec,
  defaultProps: (format, language = "fr") => ({
    ...common(format, "Priere", language),
    kicker: label(language, "kickerPriere"),
...
const devotion = define<DevotionProps>({
  ...
  spec: devotionSpec,
  defaultProps: (format, language = "fr") => ({
    ...common(format, "Devotion", language),
    kicker: label(language, "kickerDevotion"),
    ...
    callToAction: label(language, "ctaDevotion"),
    showVersion: true,
  }),
});
...
const citation = define<CitationProps>({
  ...
  spec: citationSpec,
  defaultProps: (format, language = "fr") => ({
    ...common(format, "Citation", language),
    kicker: label(language, "kickerCitation"),
...
const evenement = define<EvenementProps>({
  ...
  spec: evenementSpec,
  defaultProps: (format, language = "fr") => ({
    ...common(format, "Evenement", language),
    kicker: label(language, "kickerEvenement"),
    ...
    callToAction: label(language, "ctaEvenement"),
  }),
});
